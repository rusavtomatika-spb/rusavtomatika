<?php
global $arr_cloud_tags;
$arr_cloud_tags = [
    [
        'name' => 'Встраиваемые промышленные компьютеры',
        'url' => '/catalog/embedded_computers/',
        'add_to_sitemap' => 0,
        'category' => 'computers',
    ],
    [
        'name' => 'Панельные промышленные компьютеры',
        'url' => '/catalog/panel_computers/?&brand=aplex',
        'add_to_sitemap' => 0,
        'category' => 'computers',
    ],
    [
        'name' => 'Сенсорные промышленные мониторы',
        'url' => '/catalog/industrial_monitors/',
        'add_to_sitemap' => 0,
        'category' => 'monitors',
    ],
    [
        'name' => 'Панель оператора с ethernet',
        'url' => '/catalog/operator_panels/?&interfaces=ethernet',
        'add_to_sitemap' => 0,
        'category' => 'hmi',
    ],
    [
        'name' => 'Панель оператора с Codesys',
        'url' => '/catalog/operator_panels/?&codesys=yes',
        'add_to_sitemap' => 0,
        'category' => 'hmi',
    ],
    [
        'name' => "Панель оператора со встроенным ПЛК",
        'url' => '/catalog/operator_panels/?&codesys=yes',
        'add_to_sitemap' => 0,
        'category' => 'hmi',
    ],
    [
        'name' => 'Панель оператора Weintek 4.3 дюйма',
        'url' => '/catalog/operator_panels/?&range_diagonal_max=4.3&brand=Weintek',
        'add_to_sitemap' => 1,
        'category' => 'hmi',
    ],
    [
        'name' => 'Панель оператора с WiFi',
        'url' => '/catalog/operator_panels/?&interfaces=wifi',
        'add_to_sitemap' => 0,
        'category' => 'hmi',
    ],
    [
        'name' => 'Панель оператора с EasyAccess',
        'url' => '/catalog/operator_panels/?&brand=weintek',
        'add_to_sitemap' => 0,
        'category' => 'hmi',
    ],
    [
        'name' => 'Панель оператора с емкостным сенсором',
        'url' => '/catalog/operator_panels/?&sensor_type=capacitive',
        'add_to_sitemap' => 0,
        'category' => 'hmi',
    ],
    [
        'name' => 'Панель оператора с резистивным сенсором',
        'url' => '/catalog/operator_panels/?&sensor_type=resistive',
        'add_to_sitemap' => 0,
        'category' => 'hmi',
    ],
    [
        'name' => 'Панель оператора с sd-картой',
        'url' => '/catalog/operator_panels/?&sdcard=yes',
        'add_to_sitemap' => 0,
        'category' => 'hmi',
    ],
    [
        'name' => 'Панель оператора Weintek 15 дюймов',
        'url' => '/catalog/operator_panels/?&range_diagonal_min=15&brand=Weintek',
        'add_to_sitemap' => 1,
        'category' => 'hmi',
    ],
    [
        'name' => 'Панель оператора с просмотром изображения с ПК',
        'url' => '/catalog/operator_panels/?&cmtviewer=yes',
        'add_to_sitemap' => 0,
        'category' => 'hmi',
    ],
    [
        'name' => 'Панель оператора Weintek 12 дюймов',
        'url' => '/catalog/operator_panels/?&range_diagonal_min=12&range_diagonal_max=12.5&brand=Weintek',
        'add_to_sitemap' => 1,
        'category' => 'hmi',
    ],
    [
        'name' => 'Панель оператора с JavaScript',
        'url' => '/catalog/operator_panels/?&series=cMT-X',
        'add_to_sitemap' => 0,
        'category' => 'hmi',
    ],
    [
        'name' => 'Панель оператора с подключением IP камеры',
        'url' => '/catalog/operator_panels/?&camera_ip=yes',
        'add_to_sitemap' => 0,
        'category' => 'hmi',
    ],
    [
        'name' => 'Панель оператора с подключением USB камеры',
        'url' => '/catalog/operator_panels/?&camera_usb=yes',
        'add_to_sitemap' => 0,
        'category' => 'hmi',
    ],
    [
        'name' => 'Панель оператора Weintek 10 дюймов',
        'url' => '/catalog/operator_panels/?&range_diagonal_min=9&range_diagonal_max=10.5&brand=Weintek',
        'add_to_sitemap' => 1,
        'category' => 'hmi',
    ],
    [
        'name' => 'Удаленное управление панелью оператора через веб-браузер',
        'url' => '/catalog/operator_panels/?&webview=yes',
        'add_to_sitemap' => 0,
        'category' => 'hmi',
    ],
    [
        'name' => 'Панель со встроенным веб-сервером',
        'url' => '/catalog/operator_panels/?&plc_web_browser=yes',
        'add_to_sitemap' => 0,
        'category' => 'hmi',
    ],
    [
        'name' => 'Панель оператора на ТВ',
        'url' => '/catalog/operator_panels/?&interfaces=hdmi',
        'add_to_sitemap' => 0,
        'category' => 'hmi',
    ],
    [
        'name' => 'Панель оператора Full HD',
        'url' => '/catalog/operator_panels/?&resolutions=1920x1080,3600x1800',
        'add_to_sitemap' => 0,
        'category' => 'hmi',
    ],
    [
        'name' => 'Панель оператора Weintek 7 дюймов',
        'url' => '/catalog/operator_panels/?&range_diagonal_min=7&range_diagonal_max=7.5&brand=Weintek',
        'add_to_sitemap' => 1,
        'category' => 'hmi',
    ],
];
