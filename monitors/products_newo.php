...
if ($r->type == 'open_hmi') {
    $type = 'OPEN HMI';
    $title = 'Open HMI ' . $r->model . ' ' . $r->brand . ' со склада по отличной цене';
};
...
if ($r->type == 'cloud_hmi') {
    ...
    $title = 'Облачный интерфейс ' . $r->model . ' ' . $r->brand . ' со склада по отличной цене';
...
if ($r->type == 'monitor') {
    ...
    $keywords = 'Промышленный, монитор, ' . $r->model . ', ' . $r->brand . ', диагональ ' . $r->diagonal . '&quot;, встраиваемый, сенсорный';
...
$nav = '<br /><nav><a href="/">Главная</a>&nbsp;/&nbsp;<a href="/monitors/">Промышленные мониторы</a>&nbsp;/&nbsp;Промышленные монитор ' . (($r->brand != 'IFC') ? $r->brand : '') . ' ' . $r->model . '</nav>';
...
<tr><td>' . $im . '</td><td ' . $he . '>Hапряжение питания монитора 12-24 VDC, в комплекте идет блок питания </td></tr>
...
';
    ...
    $poverplag = '<tr><td>' . $im . '</td><td ' . $he . '>Резистивный / емкостной сенсор </td></tr>
<tr><td>' . $im . '</td><td ' . $he . '>Выход сенсора экрана — USB</td></tr>
<tr><td>' . $im . '</td><td ' . $he . '>Hапряжение питания монитора 12 VDC (блок питания - опция)</td></tr>
<tr><td>' . $im . '</td><td ' . $he . '>Пылевлагозащита по фронту ( IP65 )</td></tr>
';
}
;
...
if (!empty($modifi)) {
    $prices_out .= '<br><span class=big_pan_name><b>Возможные модификации:</b></span> <br><br>' . $modifi;
}
//----------------------------tab1 -----------------------------------------
...
if ($r->type == 'monitor') {
    ...
    if ($r->brand == 'IFC') {
        if ($r->diagonal >= 10) {
            ...
            $interfaces = $us_tr_l . 'Видео-входы</td>' . $us_tr_l1 . $outputs__video__all__string . '</td></tr>' .
        ...
        $interfaces = $us_tr_l . 'Видео-входы</td>' . $us_tr_l1 . 'VGA + DVI-D</td></tr>' .
...
//if (!empty($r->vga_port)) {$vga_port = $us_tr_l.'VGA порт</td>'.$us_tr_l1.$r->vga_port.' разъем</td></tr>';} else {$vga_port='';};
        if (!empty($r->audio)) {
            $audio = $us_tr_l . 'Аудио</td>' . $us_tr_l1 . $r->audio . ' </td></tr>';
        } else {
            $audio = '';
        };
        if (!empty($r->audio)) {
            $audio .= $us_tr_l . 'Аудио вход</td>' . $us_tr_l1 . $r->mic_in . ' </td></tr>';
        } else {
            $audio = '';
        };
...
if ($r->brand == 'Cincoze') {
    ...
    if (!empty($r->netto) && !empty($r->weight)) {
        ...
        $netto = $us_tr_l . 'Вес (нетто)</td>' . $us_tr_l1 . $r->netto . '</td></tr>';
    } else {
        $netto = '';
    };
}
...
$powero = $us_tr_l . 'Напряжение питания</td>' . $us_tr_l1 . $r->voltage . ' </td></tr>';
$powero .= $us_tr_l . 'Потребляемая мощность</td>' . $us_tr_l1 . $r->power . ' Вт</td></tr>';
...
$sp_data1 .= "<span class=hpar>Качество и безопасность</span><br>
 $table_start
 <tr><td class=par_name1>Сертификация</td><td class=par_val1>$r->certification</td></tr></table><br><br>";
...
$queryd = mysqli_query($mysqli_db, $queryd) or die("ошибка запроса0213" . $queryd);
...
$query2 = mysqli_query($mysqli_db, $query2) or die("ошибка запроса0212" . $query);
...
<div id="tabs">
  ...
  <div id="tabs-1">
  <h2>Технические характеристики монитора ' . $r->model . '</h2>
    ' . $sp_data1 . '
  </div>
  <div id="tabs-2"><h2>Габариты монитора ' . $r->model . '</h2>
  ' . $dimensions . '
  </div>';
if ($r->type != 'monitor') {
    $vyvod .= '<div id="tabs-3">
...
$prices_out .= "
  <div id='tabs-4'>
  <div class=connectors>
  <h2>Файлы для работы с монитором $r->model</h2>
  </div>
