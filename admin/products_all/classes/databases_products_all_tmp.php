...
class DBWORK {
    ...
    public function add_catalog_section($arguments) {
        ...
        $this->query = "INSERT INTO `products_all_tmp`" .
    ...
    public function update_pics_number($model, $number) {
        ...
        global $mysqli_db;
        if ($model == "" ) {return;}
        $query = "UPDATE `products_all_tmp` SET `pics_number` = '$number' WHERE `model` = '$model';";
        database_connect();
        global $mysqli_db;
        mysqli_query($mysqli_db,"SET NAMES utf8");
    ...
    public function incrementing_pics_number($product_id) {
        ...
        global $mysqli_db;
        mysqli_query($mysqli_db,"SET NAMES utf8");
        echo $query = "UPDATE `products_all_tmp` SET `pics_number` = `pics_number`+1 WHERE `index` = '$product_id';";
        $result = mysqli_query($mysqli_db,$query) or die("Invalid query: " . mysqli_error($mysqli_db) . "<br>" . $query);
    ...
    public function edit_catalog_element($element_id, $arguments) {
        ...
        $query = "UPDATE `products_all_tmp` SET ";
...
//                $query .= "`" . $real_field_name . "` = '" . addslashes(html_entity_decode($arguments[$key])) . "',";
    ...
    public function edit_catalog_section($section_id, $arguments) {
        ...
        $this->query = "UPDATE `products_all_tmp` "
                . "SET "
                . "`name` = '" . $arguments['name'] . "',"
                . "`code` = '" . $arguments['code'] . "',"
                ...
                . "WHERE `products_all_tmp`.`id` = '$section_id';";
        database_connect();
        global $mysqli_db;
        mysqli_query($mysqli_db,"SET NAMES utf8");
    ...
    public function get_catalog_section() {
        ...
        database_connect();
        mysqli_query($mysqli_db,"SET NAMES utf8");
        $query = "SELECT * FROM `products_all_tmp`;";
        $result = mysqli_query($mysqli_db,$query) or die("Invalid query: " . mysqli_error($mysqli_db));
        $rows = mysqli_num_rows($result);
        if ($rows > 0) {
            for ($row = 0; $row < $rows; $row++) {
    ...
    public function get_catalog_section_by_id($section_id) {
        ...
        global $mysqli_db;
        mysqli_query($mysqli_db,"SET NAMES utf8");
        $query = "SELECT * FROM `products_all_tmp` WHERE `id` = '" . $section_id . "';";
        $result = mysqli_query($mysqli_db,$query) or die("Invalid query: " . mysqli_error($mysqli_db));
        $rows = mysqli_num_rows($result);
        if ($rows > 0) {
            for ($row = 0; $row < $rows; $row++) {
    ...
    public function get_catalog_element_by_id($index) {
        ...
        global $mysqli_db;
        mysqli_query($mysqli_db,"SET NAMES utf8");
        $query = "SELECT * FROM `products_all_tmp` WHERE `index` = " . $index . ";";
        $result = mysqli_query($mysqli_db,$query) or die("Invalid query: " . mysqli_error($mysqli_db));
        $rows = mysqli_num_rows($result);
    ...
    public function getlist_table_fields() {
        ...
        global $mysqli_db;
        mysqli_query($mysqli_db,"SET NAMES utf8");
        $result = mysqli_query($mysqli_db,"SHOW COLUMNS FROM `products_all_tmp`");
    ...
    public function get_list_catalog_elements($order_by = "") {
        ...
        if ($order_by != "") {
            if ($order_by == "index") {
                $query = "SELECT * FROM `products_all_tmp` ORDER BY `index` DESC LIMIT 1000 ";
            } elseif ($order_by == "brand") {
                $query = "SELECT * FROM `products_all_tmp` ORDER BY `brand` ASC,`index` DESC  LIMIT 1000 ";
            } elseif ($order_by == "model") {
                $query = "SELECT * FROM `products_all_tmp` ORDER BY `model` LIMIT 1000 ";
            } elseif ($order_by == "type") {
                $query = "SELECT * FROM `products_all_tmp` ORDER BY `type` LIMIT 1000 ";
            } else {
                $query = "SELECT * FROM `products_all_tmp` ORDER BY `index` DESC, `model` LIMIT 1000 ";
            }
        } else {
            $query = "SELECT * FROM `products_all_tmp` ORDER BY `index` DESC, `model` ASC LIMIT 1000 ";
        }
        //echo $query;
        //$query = "SELECT * FROM `products_all_tmp` ORDER BY `brand`, `model` LIMIT 1000 ";
        $result = mysqli_query($mysqli_db,$query) or die("Invalid query: " . mysqli_error($mysqli_db));
        $rows = mysqli_num_rows($result);
        if ($rows > 0) {
            for ($row = 0; $row < $rows; $row++) {
    ...
    public function delete_catalog_section($id) {
        if ($id > 0) {
            ...
            global $mysqli_db;
            mysqli_query($mysqli_db,"SET NAMES utf8");
            $query = "DELETE FROM `products_all_tmp` WHERE `products_all_tmp`.`id` = $id;";
            $result = mysqli_query($mysqli_db,$query) or die("Invalid query: " . mysqli_error($mysqli_db));
            if ($result) {
