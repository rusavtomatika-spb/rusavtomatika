...
$extra_openGraph = Array(
    "openGraph_image" => "https://www.rusavtomatika.com/upload_files/images/cincoze/panel_pc/" . $model_without_wrong_symbol . "/md/" . $model_without_wrong_symbol . "_1.png",
    "openGraph_title" => "Панельный промышленный компьютер " . $r->model,
...
$onst = show_stock($r, 1);
$min_table = min_ps($r->s_name . ',P2002E', 5, 10, 350); // mins_in_row, mins_max, table_width
...
<table><tr><td width=840 align=left bfcolor=#cccccc><h1 class=big_pan_name>ПАНЕЛЬНЫЙ КОМПЬЮТЕР $r->diagonal" . "&#8243; <b>$r->brand $r->model</b> </h1></td><td width=120>
 <div class=pan_price_big title='Розничная цена'> Цена с НДС </div></td>" . $price . "</tr></table>
...
<div id=cont_rp>
    ...
    <table width=100%>
        ...
        if ($r->type == "panel_pc" && $r->diagonal > 0) {
            ...
            ?>
            <tr><td><div class=but_wr><div  class='zakazbut addToCart' idm='<?= $r->model ?>'><span>В корзину</span></div></div> </td>
                ...
                <td colspan=2>
                    <div class=but_wr>
                        <div class='link_advanced_search_wrapper'>
                            <a class="link_advanced_search"  href="/advanced_search.php?diagonal=<?= $nominal_diagonal ?>&min_diagonal=<?= $search_diagonal ?>&max_diagonal=<?= $search_diagonal2 ?>&types=1">
                                <span class="link_advanced_search_icon"></span>Подобрать панельные комп. с диагональю <?= $r->diagonal ?>&Prime;
                            </a>
                        </div>
                    </div>
                </td>
    ...
    if (preg_match('/E$/i', $r->model)) {
        $add_plus = '<tr><td>' . $im . ' </td><td ' . $he . '>Расширения: 2x Full Size Mini-PCIe; 1x PCI или 1x PCIex4 разъемы, поддрежка CFM (PoE, IGN)</td></tr>';
    } else {
        $add_plus = '<tr><td>' . $im . ' </td><td ' . $he . '>Расширения: 2x Full Size Mini-PCIe, поддрежка CFM (PoE, IGN)</td></tr>';
    };
...
//  <tr><td class=par_name1>Аудио</td><td class=par_val1>$r->audio</td></tr>
    ...
    $video_outputs[] = '1xCDS, до трех дисплеев одновременно';
 ...
 $table_start
 <tr><td class=par_name1>Процессор</td><td class=par_val1>Intel® Core™ U (Skylake), 6-й / Intel® Core™ i5-6300U (3M кеш); TDP: 15 W</td></tr>
 <tr><td class=par_name1>Частота</td><td class=par_val1>до $freq ГГц</td></tr>
 <tr><td class=par_name1>Чипсет</td><td class=par_val1>$r->chipset</td></tr>
  <tr><td class=par_name1>BIOS</td><td class=par_val1>AMI 64Mbit SPI BIOS</td></tr>

 <tr><td class=par_name1>Объем ОЗУ</td><td class=par_val1>" . ($r->ram / 1000) . "Гб </td></tr>
 <tr><td class=par_name1>Тип ОЗУ</td><td class=par_val1>" . $r->ram_slots . "x" . $r->ram_type . " 260-pin SO-DIMM 2133МГц</td></tr>
 <tr><td class=par_name1>Максимально возможный объем ОЗУ</td><td class=par_val1>$ram_max</td></tr>
 <tr><td class=par_name1>Объем жесткого диска, тип</td><td class=par_val1>2x 2.5&Prime; SATA HDD/SSD Bay, поддержка RAID 0/1, 2 x mSATA (один общий с Mini-PCIe)</td></tr>
 ...
 <tr><td class=par_name1>Ethernet</td><td class=par_val1>$r->ethernet, 2x PoE+ (опционально CFM PoE Module) с IEEE 802.3at, до 25.5W на порт</td></tr>
 ...
 <tr><td class=par_name1>Способ охлаждения</td><td class=par_val1>" . $fan . ", разъем для подключения внешнего кулера</td></tr>
 <tr><td class=par_name1>Варианты установки</td><td class=par_val1>" . $mount . "</td></tr>
 <tr><td class=par_name1>Выключатель питания</td><td class=par_val1> на корпусе, разъем для удаленного управления питанием </td></tr>
 <tr><td class=par_name1>Кнопка Reset</td><td class=par_val1> на корпусе </td></tr>
