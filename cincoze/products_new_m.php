...
if ($r->type == 'monitor') {
    ...
    $title = 'Cincoze ' . $r->model . ', ' . $r->diagonal . ' промышленный монитор';
...
$nav = '<br /><nav><a href="/">Главная</a>&nbsp;/&nbsp;<a href="/monitors/">Промышленные мониторы</a>&nbsp;/&nbsp;Промышленные монитор ' . (($r->brand != 'IFC') ? $r->brand : '') . ' ' . $r->model . ' ' . $r->diagonal . '&#8243;</nav>';
...
<tr><td>' . $im . '</td><td ' . $he . '>Hапряжение питания 9~48 VDC (блок питания - опция)</td></tr>
<tr><td>' . $im . '</td><td ' . $he . '>Пылевлагозащита по фронту ( IP65 )</td></tr>
';
}
;
...
if ($r->type == 'monitor') {
    ...
    if ($r->brand == 'IFC') {
        if ($r->diagonal >= 10) {
            $interfaces = $us_tr_l . 'Видеовходы</td>' . $us_tr_l1 . 'VGA + DVI</td></tr>' .
                $us_tr_l . 'Аудиовход</td>' . $us_tr_l1 . '3,5 Jack</td></tr>' .
                $interfaces;
        } elseif ($r->diagonal >= 8) {
            $interfaces = $us_tr_l . 'Видеовход</td>' . $us_tr_l1 . 'VGA</td></tr>' .
                $us_tr_l . 'Аудиовход</td>' . $us_tr_l1 . '3,5 Jack</td></tr>' .
                $interfaces;
        } else {
            $interfaces = $us_tr_l . 'Видеовход</td>' . $us_tr_l1 . 'VGA</td></tr>' .
        ...
        $interfaces = $us_tr_l . 'Видеовходы</td>' . $us_tr_l1 . 'VGA + DVI-D</td></tr>' .
...
//if (!empty($r->vga_port)) {$vga_port = $us_tr_l.'VGA порт</td>'.$us_tr_l1.$r->vga_port.' разъем</td></tr>';} else {$vga_port='';};
        ...
        if (!empty($r->mic_in)) {
            $audio .= $us_tr_l . 'Аудиовход</td>' . $us_tr_l1 . $r->mic_in . ' </td></tr>';
        } else {
            $audio = '';
        };
...
$powero .= $us_tr_l . 'Потребляемая мощность</td>' . $us_tr_l1 . $r->power . ' В</td></tr>';
