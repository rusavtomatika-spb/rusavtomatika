  ...
  <noindex>
    <div class="filter_field_block__content">
      <div class="row server-rendered">
        ...
        foreach ( $arr_types as $types ) {
          ?>
        <div class="col-md-12"> <a data-position="<?= $types["position"] ?>" data-name="<?= $types["code"] ?>"
                           data-name_russian="<?= $types["short_name"] ?>"
                           :class="{ 'active': (filterSelectTypes == '<?= $types["code"] ?>')}"
                           @click="clicked_button_select_Types('<?= $types["code"] ?>')">
          <?= $types["short_name"] ?>
          </a></div>
        <?
        }
      ...
      <div class="row vue-rendered">
        <div class="col-md-12" v-for="(Types_item, index)  in filterSelectTypes" :key="index"> <a :class="{ 'active': (filterSelectedTypes == Types_item.name)}" @click="clicked_button_select_Types(Types_item.name)">{{Types_item.name_russian}}</a></div>
      </div>
    </div>
  </noindex>
</div>
