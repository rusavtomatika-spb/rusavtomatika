    ...
    while ( $row = mysqli_fetch_array( $result, MYSQLI_ASSOC ) ) {
      ...
      if ( isset( $arr_series[ 'arr_preview_fields' ] )and count( $arr_series[ 'arr_preview_fields' ] ) > 0 ) {
        ...
        foreach ( $arr_series[ 'arr_preview_fields' ] as $preview_field ) {
          ...
          $preview_text .= $field_name_short . ":&nbsp;<b>" . $row[ $preview_field ] . "</b>";
          if ( !empty( $field_units ) ) {
          $preview_text .= "&nbsp;" . $field_units."; ";
		  } else {
          $preview_text .= "; ";
		  }
        }
    ...
    while ( $row = mysqli_fetch_array( $result, MYSQLI_ASSOC ) ) {
      ...
      if ( isset( $arr_series[ 'arr_preview_fields' ] )and count( $arr_series[ 'arr_preview_fields' ] ) > 0 ) {
        ...
        foreach ( $arr_series[ 'arr_preview_fields' ] as $preview_field ) {
          ...
          if (
            $this->catalog_field_descriptions[ $preview_field ][ "name_short" ] == "Процессор" ||
            $this->catalog_field_descriptions[ $preview_field ][ "name_short" ] == "Разрешение"
          ) {
            $preview_text .= "<br>";
          }
