...
//            ["link" => "/sertificates/", "anchor" => "Сертификаты дистрибьютора"],
    ...
    [
        ...
        "items" => [
            ...
            ["link" => "/weintek_projects" . EX . "/?section=Macro_Sample/", "anchor" => "Демо-макросы Weintek"],
    ...
    ["title" => "Контакты",
        "items" => [
            ...
            ["link" => "tel:+88006003347", "anchor" => "8 (800) 600-33-47"],
    ...
    if ($ar_section["products_selected_by_query"] != "") {
        ...
        foreach ($ar_brands as $brand) {
            ...
            foreach ($ar_series as $series) {
                $items = CoreApplication::get_rows_from_table("products_all", "model,brand,s_name", " `type`='{$series["type"]}' and `series`='{$series["name"]}' and `brand`='{$brand["name"]}' and `show_in_cat`!='0' and `discontinued` != '1'  and `parent` = ''" . $extra_condition, " `index` desc");
