...
foreach ( $rows_from_products_discounted as $item ) {
  ...
  $productsFromAll[] = [
    ...
    "short_name" => !empty( $item[ "short_name" ] ) ? $item[ "short_name" ] : $model,
    "s_name" => !empty( $item[ "short_name" ] ) ? $item[ "short_name" ] : $model,
    "text_features" => !empty( $item[ "text_features" ] ) ? $item[ "text_features" ] : "",
    "retail_price" => $item[ "retail_price" ],
    "retail_price_hide" => $item[ "retail_price_hide" ],
    "action_price" => isset( $item[ "action_price" ] ) ? $item[ "action_price" ] : 0,
    "currency" => isset( $item[ "currency" ] ) ? $item[ "currency" ] : "RUR",
  ...
  <div class="component_wrapper">
    <div class="row" style="position: relative;">
      ...
      <div class="sale__page-wrapper">
                ...
                <div class="preview">
                  <div class="preview_image">
                    ...
                    if ( $discountPercent != 0 ) {
                      ?>
                    <span class="brand_plate"><i>-&nbsp;<? echo $discountPercent; ?>&nbsp;%</i></span>
                    <?
                    } else {
                      ?>
        ...
        <section class="component_catalog_section__panel_of_products sale__section-wrapper view-mode-tile" id="discounted-wrapper">
          <h2 style="margin: 20px 0 30px; font-size: 24px;text-align:center;">Уцененные товары, распродажа неликвидов</h2>
          <div class="series_products_tiles fixed-grid has-1-cols-mobile has-2-cols-tablet has-4-cols-desktop view-mode-tile sale__items-wrapper">
			  <div class="grid">
              ...
              <div class="tr_product_<?= $product_item["model"] ?> tile" data-type="<?=$product_item['type']?>" data-series="<?=$series?> cell">
                <div class="preview">
                  <div class="preview_image">
                    ...
                    ?>
                    <a href="<?= $detail_link ?>"> <img alt="<?= $product_item["s_name"]?> <? if ($product_item["brand"] != 'IFC') { echo $product_item["brand"];} ?> <?= $product_item["model"] ?>" loading="lazy" src="<?=$preview_picture?>"></a> </div>
                </div>
                <div class="td_short_description"><a href="<?= $detail_link; ?>"><span
                                class="model"> <? echo $product_item["model"];?> </span>
                  <div class="preview_text_block">
                    <div class="has-text-weight-bold"><? echo $product_item["s_name"]; ?></div>
                    </a>
                    <p>
                      <?
                      if ($series != '') echo '<span>Серия: ' . $series . '</span> ';
                      ?>
                    </p>
						<span class="preview_text">
							                <? if (!empty($text_features)): ?>
                  ...
                  <div class="series_products__panel_buttons">
                    <div class="price_block">
                      <div class="price_block_wrapper">
                        ...
                        <div class="noflex">
                          <?
                          if ( isset( $product_item[ 'retail_price' ] )and intval( $product_item[ 'retail_price' ] ) > 0 and $product_item[ 'model' ] != 'IFC-M-Series' ) {
                            switch ( $product_item[ 'currency' ] ) {
                              case 'USD':
                                if ( $product_item[ 'action_price' ] > 0 ) {
                                  echo '<span class="act_price"><span class="value">' . $product_item[ 'action_price' ] . '</span>';
