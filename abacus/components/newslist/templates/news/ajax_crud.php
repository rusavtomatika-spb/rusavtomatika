...
function create_element($fields)
{
    ...
    $query = "INSERT INTO news (SELECT NULL, now(), `update`, `name`, `sys_name`, `active`, `stext`, `btext`, `btext_for_rss`, `btext_for_rss_enclosures`, 0,`show_in_rss`, `title_cat`, `description`, `keywords`, `img`, `alt`, `show_picture_preview_in_detail` FROM news WHERE id = {$fields["id"]})";
    $result = mysqli_query($mysqli_db, $query) or die("INVALID QUERY: " . $query . "  " . mysqli_error($mysqli_db) . " <br>FILE: " . __FILE__ . " <br>LINE: " . __LINE__);
