...
function delete_element($id)
{
    ...
    echo $query = "DELETE FROM `news` WHERE `id` = '$id'";
    ...
    foreach ($fields as $key => $field) {
        if(is_string($field)){
            ...
            if (mb_detect_encoding($field) == "UTF-8") {
                $fields[$key] = iconv("utf-8", "windows-1251", $field);
            }
        }
    }
    $stext = mysqli_real_escape_string($mysqli_db,$fields["stext"]);
    $btext = mysqli_real_escape_string($mysqli_db,$fields["btext"]);
    $query = "UPDATE `news` SET `date`='{$fields["date"]}',`update`= NOW(),`name`='{$fields["name"]}',`sys_name`='{$fields["sys_name"]}'," .
...
function create_element($fields)
{
    ...
    $query = "INSERT INTO news (SELECT NULL, `date`, `update`, `name`, `sys_name`, `active`, `stext`, `btext`, `btext_for_rss`, `btext_for_rss_enclosures`, `show`,`show_in_rss`, `title_cat`, `description`, `keywords`, `img`, `alt`, `show_picture_preview_in_detail` FROM news WHERE id = {$fields["id"]})";
    $result = mysqli_query($mysqli_db, $query) or die("INVALID QUERY: " . $query . "  " . mysqli_error($mysqli_db) . " <br>FILE: " . __FILE__ . " <br>LINE: " . __LINE__);
