...
function update_element($fields)
{
    ...
    $query = "UPDATE `articles` SET `brand`='{$fields["brand"]}',`date`='{$fields["date"]}',`update`= NOW(),`name`='{$fields["name"]}',`sys_name`='{$fields["sys_name"]}'," .
...
function create_element($fields)
{
    ...
    $query = "INSERT INTO articles (SELECT NULL, `brand`, `date`, `update`, `parent`, `name`, `sys_name`, `stext`, `btext`, `show`, `por`, `title_cat`, `description`, `keywords`, `img`, `alt` FROM articles WHERE id = {$fields["id"]})";
    $result = mysqli_query($mysqli_db, $query) or die("INVALID QUERY: " . $query . "  " . mysqli_error($mysqli_db) . " <br>FILE: " . __FILE__ . " <br>LINE: " . __LINE__);
