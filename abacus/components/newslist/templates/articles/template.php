...
$TITLE = "Всё, что необходимо знать об работе с продукцией Weintek, IFC, Aplex, eWON, Samkoon".$page_n;
$CANONICAL = "https://www.rusavtomatika.com/articles/".$page_num;
$DESCRIPTION = 'Статьи наших экспертов помогут быстро и качественно разобраться в нюансах продукции Weintek, IFC, Aplex, eWON, Samkoon'.$page_n;
...
<div id="vue_app_articles" style="display: none">
  ...
  <div id="admin_panel" v-cloak>
    ...
    <div class="switch_edit_mode__wrapper">
      <div> <span>Режим редактирования включен</span>
        <button class="switch_edit_mode__button active" @click="switch_edit_mode__button_clicked"> Выключить </button>
      </div>
    </div>
    <div v-if="edit_panel_show" class="edit_panel">
      ...
      <div class="edit_panel_item buttons"> <span>Редактирование статьи {{current_edit_item.id}}</span>
        ...
        <button @click="edit_save_and_close_element_button__clicked(current_edit_item.id)"> Сохранить и закрыть </button>
      ...
      <div class="edit_panel_item img_wrap"> <span>Изображение анонса</span> <span><img style="max-height: 50px;margin-right: 10px;" :src="current_edit_item.img"></span>
      ...
      <div class="edit_panel_item stext_wrap"><span>Анонс</span>
        <textarea v-model="current_edit_item.stext" name="" id="" cols="30" rows="10"></textarea>
      </div>
      <div class="edit_panel_item btext_wrap"><span>Подробно</span> 
        ...
        <textarea v-model="current_edit_item.btext" name="" id="mytextarea" cols="30"
                                rows="10">
</textarea>
      </div>
      <div class="edit_panel_item"> <span>Доп. Изображения</span>
        <div style="width: 100%;box-shadow: 0 0 5px rgba(0,0,0,0.5) inset;"> <span class="edit_panel_item__more_images"
                                  v-for="(image_item, image_index) in current_edit_item.images"> <img style="height:40px" :src="image_item"> <span>{{image_item}}</span>
          <button class="button_delete red" @click="show_popup_delete_confirm_file(image_item)">Удалить</button>
          </span> </div>
        <div style="padding: 10px;">
          <div> <span>[*.jpg, *.png, *.gif, *.webp, *.zip]</span>
            <input type="file" accept="image/jpeg,image/png,image/gif,image/webp,application/zip" id="image_file2" ref="file2"
                                       v-on:change="handleFileUpload()"/>
          </div>
        </div>
        <div style="padding: 10px;">
      ...
      <div class="edit_panel_item buttons"> <span>Редактирование статьи</span>
        <button @click="edit_restore_element_button__clicked(current_edit_item.id)">Восстановить </button>
    ...
    <?php
      ...
      ?>
    <div class="switch_edit_mode__wrapper">
      <div> <span>Режим редактирования выключен</span>
        <button class="switch_edit_mode__button" @click="switch_edit_mode__button_clicked"> Включить </button>
      </div>
      <div></div>
    </div>
    <?php } ?>
        ...
        <div class="row">
          <div :key="product_index"
            ...
            <div class="item" :class=" 'show_' + product_item.show ">
              <div class="preview_image_wrapper"> <a :href="product_item.link">
              ...
              <div class="edit_element_buttons_wrapper">
                <div>
                  <button @click="edit_element_button__clicked(product_item)"> Редактировать </button>
                </div>
                <div>
                  <div>
                    <button @click="copy_element_button__clicked(product_item)"> Копировать </button>
                  </div>
                  <div>
                    <button v-if="product_item.show == '1'"
                                                    @click="deactivate_element_button_clicked(product_item)"> Деактивировать </button>
                    <button v-if="product_item.show == '0'"
                                                    @click="activate_element_button_clicked(product_item)">Активировать </button>
                  </div>
                  <div>
                    <button @click="deleting_element_index=product_index; show_popup_delete_confirm_message(product_item)"> Удалить </button>
                  </div>
                </div>
              </div>
              <?
          ...
          <div class="col-md-12" v-if="arr_pager_items.length > 0">
            <div class="pager_wrapper">
              ...
              <div :key="pager_index" :class="['pager_item', {'active': pager_item.active}]"
                                 v-for="(pager_item, pager_index) in arr_pager_items"> <a v-if="!pager_item.active"
                                   :href="(pager_item.link.length > 0)?pager_item.link:'/articles/'">{{pager_item.name}}</a> <span v-if="pager_item.active">{{pager_item.name}}</span> </div>
            </div>
          </div>
        </div>
      </div>
