...
if (!$do_not_show_pager) {
    $rows = get_rows_from_table("`articles`", "", $filter_str, "date DESC", $offset . "," . $number_elements_per_page);
} else {
    $rows = get_rows_from_table("`articles`", "", $filter_str, "date DESC");
}
foreach ($rows as $key => $row) {
    $rows[$key]['date'] = date('d.m.Y', strtotime($row['date']));
    $rows[$key]['link'] = "/articles/".$row['sys_name'] . "/";
...
if (!$do_not_show_pager) {
    $arr_pager_items = get_pager("articles", $filter_str, $number_elements_per_page, $current_page);
...
$out["buffer"] = $buffer;
$out["products"] = $rows;
$out["all_brands"] = get_all_brands("articles");
...
function get_list_of_images($id)
{
    ...
    if (is_array($files)) {
        foreach ($files as $file) {
            $full_name = $dir . '/' . $file;
            if (mime_content_type($full_name) == 'image/jpeg' or mime_content_type($full_name) == 'image/png' or mime_content_type($full_name) == 'image/webp'  or mime_content_type($full_name) == 'application/octet-stream') {
