                        ...
                        <div class="horizontal_slider_default">
                            ...
                            <div id="carouselhAuto">
                                ...
                                foreach ($arrLinkedModels as $model) {
                                    ...
                                    $counter_totalItems++;
									$fname = str_replace(array(')', '(', '/',' '),  array('\)', '\(', '_', '_'), $model["preview_image"]);                    ?>
                                    ?>
                                    <div class="panelLinkedModels_item">
                                        ...
                                        <div class="card-image">
                                            <figure class="image is-4by3">
                                                <img src="<?/*= $model["preview_image"] */ ?>" alt="Placeholder image">
                                            </figure>
                                        </div>
...
-->
                                            <span class="panelLinkedModels_picture"
                                                  style="background-image: url('<?= $model["preview_image"] ?>')"></span>
            ...
            <style type="text/css">
                ...
                .jscarousal-horizontal {
                    width: 100%; /* ширина и высота коробки для слайдов */
            ...
            <script src="/cms/components/vertical_slider/templates/default/jsCarousel-2.0.0.js"
...
//Вертикальная
                        $('#carouselhAuto').jsCarousel({
                            ...
                            scrollspeed: 600, //время эффекта прокрутки
                            delay: 7000, //время прокрутки слайдов
    ...
    {
        $query = "SELECT `model`,`type`,`brand` FROM `products_all` where `model` = '$model';";
        $result = mysqli_query(self::$_db, $query) or die("Invalid query: " . $query . " error: " . mysqli_error(self::$_db));
        if (mysqli_num_rows($result) == 0) {
            return array();
        }
        ...
        $out = '';
        if ($element) {
            $element['brand'] = strtolower($element['brand']);
            $element['type'] = strtolower($element['type']);
			$fname = str_replace(array(')', '(', '/',' '),  array('\)', '\(', '_', '_'), $element['model']);                    
            $out = "/images/{$element['brand']}/{$element['type']}/{$fname}/130/{$fname}_1.webp";
        }
