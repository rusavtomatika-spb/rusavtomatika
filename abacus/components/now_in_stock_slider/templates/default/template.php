...
$arResult["items"] = CoreApplication::get_rows_from_table("`products_all`",
    ...
    "model,s_name,diagonal,brand,type,retail_price,retail_price_hide,currency,model_fullname,show_on_stock,series",

    "`brand` != 'Faraday' and `onstock_spb` > 0 and `discontinued` != '1' and `parent`='' and `recommended`> 0 and `status` != '0'", " rand() ", 28);//`index` DESC
