...
//if($mode=="") die(iconv( "CP1251","UTF-8","<div style='height:300px;'>неправильный запрос</div>"));
    ...
    if ($m0 != "") {
        ...
        $sql = "SELECT * FROM products_all WHERE `model` = '$m0' AND `discontinued`!='1' ";
...
//die(iconv( "CP1251","UTF-8","<div style='height:300px;'>неправильный запрос</div>"));
    ...
    if ($m1 != "") {
        $sql = "SELECT * FROM products_all WHERE `model` = '$m1'  AND `discontinued`!='1' ";
        $res = mysqli_query($mysqli_db,$sql) or die(mysqli_error($mysqli_db));
        $row = mysqli_fetch_assoc($res);
        $k1["name"] = $row["model"];
        $k1["type"] = $row["type"];
...
//die(iconv( "CP1251","UTF-8","<div style='height:300px;'>неправильный запрос</div>"));
    ...
    if ($m2 != "") {
        $sql = "SELECT * FROM products_all WHERE `model` = '$m2'  AND `discontinued`!='1' ";
        $res = mysqli_query($mysqli_db,$sql) or die(mysqli_error($mysqli_db));
        $row = mysqli_fetch_assoc($res);
        $k2["name"] = $row["model"];
        $k2["type"] = $row["type"];
...
//die(iconv( "CP1251","UTF-8","<div style='height:300px;'>неправильный запрос</div>"));
    ...
    if ($m3 != "") {
        $sql = "SELECT * FROM products_all WHERE `model` = '$m3'  AND `discontinued`!='1' ";
        $res = mysqli_query($mysqli_db,$sql) or die(mysqli_error($mysqli_db));
        $row = mysqli_fetch_assoc($res);
        $k3["name"] = $row["model"];
        $k3["type"] = $row["type"];
...
//<tr><td width=225 valign=top> $p0 </td><td width=225 valign=top> $p1</td>  <td width=225 valign=top> $p2</td>  <td valign=top> $p3</td>  </tr>
    ...
    if (!empty($new_path_picture)) {
      ...
      $uk0 = "/images/" . mb_strtolower($k0["brand"]) . "/" . mb_strtolower($k0["type"]) . "/" . $k0["name"] . "/md/" . $k0["name"] . "_1.png";
      $uk1 = "/images/" . mb_strtolower($k1["brand"]) . "/" . mb_strtolower($k1["type"]) . "/" . $k1["name"] . "/md/" . $k1["name"] . "_1.png";
      $uk2 = "/images/" . mb_strtolower($k2["brand"]) . "/" . mb_strtolower($k2["type"]) . "/" . $k2["name"] . "/md/" . $k2["name"] . "_1.png";
      $uk3 = "/images/" . mb_strtolower($k3["brand"]) . "/" . mb_strtolower($k3["type"]) . "/" . $k3["name"] . "/md/" . $k3["name"] . "_1.png";
