...
if ($r->type == 'coupler') {
    ...
    if ($kolvo > 1) {
        ...
        $type = 'Коммуникационные модули (Coupler) для модульной системы ввода вывода (Coupler + Digital I/O)';

    } else {

        $type = 'Коммуникационный модуль (Coupler) для модульной системы ввода вывода (Coupler + Digital I/O)';
...
if ($r->AI > 0 || $r->AO > 0) {
    ...
    $module_type = 'управлением движения';
...
if ($r->type == 'module') {
    ...
    if ($kolvo > 1) {
        ...
        if (!empty($r->pulse_input) || !empty($r->pulse_output)) {
            ...
            $type = 'Модуль для управления движением';
...
if (!empty($r->meta_h1)) {
    ...
    $nav = '<br /><nav><a href="/">Главная</a>&nbsp;/&nbsp;<a href="/plc/weintek/">Модульные системы Weintek</a>&nbsp;/&nbsp;' . $title . '</nav>';
...
";
            ...
            if (!empty($r->max_connection)) {
                ...
                $max_connection = "<li class='urlup' data='max_connection'><a href='#tabs-6'>КОЛИЧЕСТВО ПОДКЛЮЧАЕМЫХ МОДУЛЕЙ</a></li>";
