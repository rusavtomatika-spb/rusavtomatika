    ...
    foreach ( $array as $element ) {
      ...
      if ( strpos( $element[ $key ], $value ) !== false ) {
        ...
        if ( preg_match( '/\.package$/', $element[ 'fname' ] ) ) {
          $folder = 'https://'.$_SERVER['HTTP_HOST'].'/download/soft';
          $fput = '';
        }

if ($out=='download') {
        echo '<p><a class="download_' . strtolower( $info[ 'extension' ] ) . '" href="' . $folder . '/' . $fput . $element[ 'fname' ] . '" download="' . $element[ 'fname' ] . '">' . $element[ 'title' ] . '</a> <span class="small_gray_text">' . $mod_data . ' ' . $fsize . '</span></p>';
} else {
echo '<tr><td><div><a download="" title="Скачать" target="_blank" href="' . $folder . '/' . $fput . $element[ 'fname' ] . '">' . $element[ 'title' ] . '</a></div><div class="down_item_descr">' . $element[ 'description' ] . '</div></td><td style="font-size: 0.8em; white-space: nowrap;text-transform: uppercase;">' . $element[ 'ftype' ] . '</td> <td style="font-size: 0.8em; white-space: nowrap;"><span class="date">' . $mod_data . '</span></td> <td style="font-size: 0.8em;"><span class="size">' . $fsize . '</span></td> <td style="font-size: 0.8em; text-transform: uppercase;"><span class="language">eng</span></td><td style="display: none;">' . $element[ 'sort' ] . '</td></tr>';	
}
      } else continue;
    }
  }
  ...
  if ( is_array( $array ) ) {
    foreach ( $array as $element ) {
      ...
      if ( $el_title = 'EasyBuilder Pro User Manual (All chapters)(English)' ) {
        $el_title = 'Руководство по EasyBuilderPro ' . file_get_contents( 'https://www.rusavtomatika.com/upload_files/soft/EBPro/Installer/version.txt' );
      }
if ($out=='download') {
      echo '<p><a class="download_' . $info[ 'extension' ] . '" href="https://www.rusavtomatika.com/upload_files/documents/weintek/' . $element[ 'put' ] . '/' . $element[ 'fname' ] . '" download="' . $element[ 'fname' ] . '">' . $el_title . '</a> <span class="small_gray_text">[' . date( "d.m.Y", $element[ 'mod_data' ] ) . '] ' . $fsize . '</span></p>';
} else {
            ...
            <div class="block_padding">
              ...
              arr_out( $ebpro_docs );
				...
				//$ebpro_files .= '<tr><td class="is-size-7 has-text-danger">On-line руководство пользователя Weintek EasyBuilder Pro (Eng) </td><td class="is-size-7 has-text-danger"><a href="/weintek-easybuilderpro-user-manual-en/">link</a></td><td class="is-size-7 has-text-danger">'. date("d.m.Y",file_get_contents("https://www.rusavtomatika.com/upload_files/manuals/weintek/UserManual_separate_chapter/update_date.txt")).'</td><td></td></tr>';
  				$ebpro_files .= '<tr><td><div><a title="Скачать" target="_blank" href="/weintek-easybuilderpro-user-manual-en/">On-line руководство пользователя Weintek EasyBuilder Pro (Eng)</a></div><div class="down_item_descr">Приложение EasyBuilderPro применяется для создания пользовательских проектов для операторских панелей Weintek
</div></td> <td style="font-size: 0.8em; white-space: nowrap;text-transform: uppercase;">WEB</td> <td style="font-size: 0.8em; white-space: nowrap;"><span class="date">['. date("d.m.Y",file_get_contents("https://www.rusavtomatika.com/upload_files/manuals/weintek/UserManual_separate_chapter/update_date.txt")).']</span></td> <td style="font-size: 0.8em;"></td> <td style="font-size: 0.8em; text-transform: uppercase;"><span class="language">Eng</span></td><td style="display: none;">1550</td></tr>';	
