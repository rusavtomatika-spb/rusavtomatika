...
$description = $r->model . ', ' . $r->brand . ', панели оператора, операторские панели, склад в Санкт-Петербурге, склад в Москве, склад в Киеве, низкие цены ';
if ($r->type == 'cloud_hmi') {
    $type = 'ОБЛАЧНЫЙ ИНТЕРФЕЙС';
    $title = $r->model . ', облачный интерфейс ' . $r->brand;
...
if ($r->type == 'monitor_cloud_hmi') {
    ...
    $title = $r->model . ', экран облачного интерфейса ' . $r->brand;
...
if ($r->type == 'monitor') {
    ...
    $title = 'Промышленный монитор ' . $r->diagonal . '&#8243; ( встраиваемый, сенсорный ) ' . $r->model;
...
if ($r->model == 'cMT-SVR-100' or $r->model == 'cMT-HDMI') {
...
<table><tr><td width=840 align=left><h1 class=\"big_pan_name" . $class . "\">$h1</h1></td><td width=120>
 <div class=pan_price_big title='Розничная цена'> Цена с НДС </div></td>" . $priceb . "</tr></table>
<div class=hc1>&nbsp;</div>" . $analog . "<br>
</td></tr>
<tr ><td valign=top align=center valign=center width=450px>
...
if ($r->diagonal > 0) {
 ...
 <tr><td class=par_name1>Разрешение</td><td class=par_val1 >' . $r->resolution . '</td></tr>' .
    ...
    $sp_data1 = ' <div style="width:90%;">' . $table_start . ' </table>';
}
...
$komplect = '';
if ($r->diagonal == '7' or ( $r->diagonal >= 9 and $r->diagonal < 10 and $r->model != 'cMT-iV5')) {
...
if ($r->diagonal >= 9 and $r->diagonal < 10 and $r->model != 'cMT-iV5') {
    ob_start();
    ?>
