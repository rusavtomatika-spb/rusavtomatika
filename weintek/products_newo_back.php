...
//echo $_SERVER['DOCUMENT_ROOT'];
            ...
            if ($r->type == 'monitor') {
                $path = 'img/panels/';
                if (file_exists('/home/weblec/public_html/ifc-ipc.cn/' . $path . $img_row[s_img_path])) {
                    $im1 = $path . $img_row[s_img_path];
                };
            } elseif ($r->type == 'cloud_hmi') {
                $path = 'image/';

                if (file_exists('/home/weblec/public_html/test_ra/' . $path . $img_row[s_img_path])) {
                    $im1 = $path . $img_row[s_img_path];
                };
            };
...
if (($r->type == 'monitor') AND ( $r->model != 'IFC-M57')) {
    $glas = "<tr><td>$im </td><td $he>Тонкое модульное исполнение и прочная конструкция</td></tr>";
};
if (($r->type == 'monitor')) {
    $glas1 = "<tr><td>$im </td><td $he>6мм передняя панель</td></tr>";
};
if (($r->type == 'monitor') AND ( $r->model != 'IFC-M57') AND ( $r->model != 'IFC-M8')) {
...
$sp_data1 .= '<span class=hpar>Интерфейсы</span><br>' .
        $table_start .
        $ethernet_full . $serial_full . $usb_host . $serial . $parallel_port . $pci_slot . $ps2 . $mic_in . $linear_out .
        $speakers . $vga_port . $audio . $voltage . $rgb . $dc_input . $av_input . $dvi_d . $ts_rs232 . $ts_usb . $s_video . '</table><br><br>';
...
if ($j2 > 0) {
    for ($i2 = 0; $i2 < $j2; $i2++) {
        ...
        $sname = str_replace("\r\n", "<br />", $row2[name]);
        $scheme .= '<div class="com"><div class="com_about">Название разъема:<br />' . $sname . '<br /><br />Тип:<br />' . $row2[type] . '<img class="" src="/images/' . $row2[image] . '" /><br /></div>';
...
if ($r->type != 'monitor') {
  ...
  <h2>COM-порты панели оператора ' . $r->model . '</h2>' . $scheme . '
  </div><br><br></div>
  ';
};
...
if (file_exists('/home/weblec/public_html/test_weinteknet/manuals/' . $r->model . '.pdf')) {
    $soft5 = "
...
if (file_exists('/home/weblec/public_html/test_weinteknet/install/' . $r->model . '.pdf')) {
    $soft6 = "
 <tr><td><div class=down_item> Руководство по установке " . $types . "а $r->model </div></td>
...
if (file_exists('../monitors/manuals/' . $r->model . '.pdf')) {
    $soft7 = "
 <tr><td><div class=down_item> Брошюра (datasheet) " . $types . "а $r->model </div></td>
   <td><div class=dt_item>" . get_file_date('../monitors/manuals/' . $r->model . '.pdf') . "<br>(eng)</div></td>
