...
<table><tr><td width=840 align=left bfcolor=#cccccc><h1 class=\"big_pan_name" . $class . "\">ПАНЕЛЬ ОПЕРАТОРА $r->diagonal\" <b>$r->brand $r->model</b> </h1></td><td width=50>
...
<tr><td width=30>$im </td><td $he>Высочайшее качество и надежность</td></tr>
...
$sp_data1 = "
 ...
 <tr><td class=par_name1 $bg1>Последовательные интерфейсы</td><td class=par_val1 $bg1>$r->serial_full</td></tr>
 ...
 <tr><td class=par_name1 $bg2>Слот SD карты</td><td class=par_val1 $bg2>" . ($r->sdcard ? $r->sdcard : '-') . "</td></tr>

 <tr><td class=par_name1 $bg1>Ethernet</td><td class=par_val1 $bg1>" . ($r->ethernet ? $r->ethernet : '-') . "</td></tr>

 <tr><td class=par_name1 $bg2>Линейный аудиовыход</td><td class=par_val1 $bg2>" . ($r->linear_out ? $r->linear_out : '-') . "</td></tr>

 <tr><td class=par_name1 $bg1>Видеовход</td><td class=par_val1 $bg1>" . ($r->video_input ? $r->video_input : '-') . "</td></tr>
...
<?else:?>
    ...
    <?if($r->codesys != 0):?>
        ...
        <tr><td class=par_name1 <?=$bg1?>>Поддержка Codesys</td><td class=par_val1 <?=$bg1?>>Есть</td></tr>
    ...
    пользовательские исполняемые файлы. Программист может пользоваться только теми возможностями, которые предоставляет <?=$r->software?></td></tr>
...
<div id='tabs'>
  ...
  <h2>COM-порты панели оператора $r->model</h2><br />
...
echo "<tr><td><div class=down_item> Брошюра на панель оператора $r->model </div></td>
...
if ($r->user_manual) {
    ...
    echo "<tr><td><div class=down_item> Руководство пользователя $r->model </div></td>
...
echo "<td><div class=down_item> Руководство по установке операторской панели $r->model </div></td>";
