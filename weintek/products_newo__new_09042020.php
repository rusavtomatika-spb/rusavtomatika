...
$soft10 = $soft__codesys = $soft__remoteio = $soft9 = $flash = $donad = $tempsto = $vibr = $fsma = $fsma = $mic_in
     = $linear_out = $speakers = $power_isolation = $voltage_resistance = $isolation_resistance = $wifi = $serial_full = $usb_host = $usb_client = $parallel_port = $can_bus = $video_input = $usb_cam
         ...
         = $fsso = $cpu_speed = $cpu_type = $ram = $current = $certification =  "";
...
$dimensions = '';
echo $queryd = "SELECT * FROM `dimensions` WHERE `sys_name`='{$r->model}' LIMIT 1;";

global $mysqli_db;
$queryd = mysqli_query($mysqli_db, $queryd) or die("<br>Ошибка запроса в файле product_newo.php !!!<br>" . $queryd . " Error: " . mysqli_error($mysqli_db));
...
$head = head($template_file, $r->title, $r->description, $r->keywords);
