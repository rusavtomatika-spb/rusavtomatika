...
$r = mysql_fetch_object($res);
    ...
    $type = 'Модуль аудио оповещения';
    $title = 'Модуль аудио оповещения ' . $r->model . ' для вывода звука в панелях Weintek';
    $keywords = 'Weintek, MT8150XE, вывод звука, Weintek audio, Weintek mp3, проиграть mp3, HMI mp3, HMI audio output, операторская панель mp3, операторская панель аудио, DAO1EM';
    $description = 'Модуль аудио оповещения ' . $r->model . '. Воспроизводит звуковые фрагменты с внешнего хостинга и с SD карты памяти, а также непрерывный поток аудио данных.';
...
if (($r->type == 'audio_alert_module')) {
    $types = 'модуль аудио оповещения';
    $types1 = 'модуля аудио оповещения';
    $typ = 'модуль';
    $typ1 = 'модуля';
    $typ2 = 'модулем';
    $typ3 = 'этим модулем';
...
<div id=dd style='height:300px;' class='main_image'>
$kartinka</div>
...
if ($link_advanced_search):
    ...
    <table width=100%>
        <? if ($r->discontinued != '1') { ?>
            <tr>
                <td><div class=but_wr><div  class='zakazbut addToCart' idm='<?= $r->model ?>'><span>В корзину</span></div></div> </td>
...
<div id="tabs">
  ...
  <ul>
    <li class="urlup" data="text_detail"><a href="#tabs-0">ПОДРОБНОЕ ОПИСАНИЕ</a></li>
  ...
  <div id="tabs-0" class="text_detail">

  <h2>Подробное описание ' . $typ1 . ' ' . $r->model . '</h2>
    ' . $r->text_detail . '
...
if (!empty($_GET[tab])) {
    ...
    } else if ($_GET[tab] == 'text_detail') {
        $uuu = '$(\'a[href="#tabs-0"]\').click();';
    }
