...
$soft10 = $soft__codesys = $soft__remoteio = $soft9 = $flash = $donad = $tempsto = $vibr = $fsma = $fsma = $mic_in
     = $linear_out = $speakers = $power_isolation = $voltage_resistance = $isolation_resistance = $wifi = $serial_full = $usb_host = $usb_client = $parallel_port = $can_bus = $video_input = $usb_cam
         = $vga_port = $hdmi = $pci_slot = $ps2 = $rgb = $dc_input = $av_input = $dvi_d = $ts_rs232 = $ts_usb = $s_video
         = $fsso = $cpu_speed = $cpu_type = $ram = $current = $certification =  $kartinka = $codesys = $webview= "";
...
//echo $r->pics_number;
//$min_table = miniatures($r->model, 6, 10, 350, $r->view3d); // mins_in_row, mins_max, table_width
...
if (($r->retail_price != '0') AND ($r->retail_price != '') AND ($r->retail_price_hide == '0')) {
    ...
    $priceb = "<td><p class='zakazbut' idm='$r->model' onclick='show_backup_call(6, \"$r->model\")'>Запросить &nbsp;цену</p></td>";
};
...
if (isset($technological) and is_array($technological)) {
...
if (isset($technologicalResult) and is_array($technologicalResult)) {
...
if (isset($r->youtube_video) && $r->youtube_video != '') {
...
<?= $nav ?>
    ...
    <br>
        ...
        <table class="product_detail_weintek" width="100%" height="400px">
    <tr>
...
?>
    ...
    <td valign="top" align="center" valign="center" width="450px">
        <?// Картинки ?>
    <?
        echo print_pictures_in_detail_template($r->brand, $r->type, $r->model, $r->pics_number);
        echo $youtube_video;
    ?>
...
$prices_out .= "
<div id='cont_rp' class='detail_section_1_block_right'>
...
if ($link_advanced_search):
    ...
    <table width=100%>
        <? if ($r->discontinued != '1') { ?>
            <tr>
                <? if ($r->retail_price > 0 and $r->retail_price_hide == '0'): ?>
                    <td>
                        <div class=but_wr>
...
if ($r->ethernet != "" && $r->os == ""){
    ...
    if ($r->ethernet != "" && $r->os != "" && strripos($r->model, "cMT-SVR") === FALSE){
...
if ($r->codesys == "optional") {
    $codesys = "<tr><td>$im </td><td $he>Лицензия CODESYS покупается отдельно. Уточняйте у наших менеджеров.</td></tr>";
} elseif ($r->codesys == "build_in") {
    $codesys = "<tr><td>$im </td><td $he>Лицензия CODESYS входит в стоимость.</td></tr>";
...
if (($r->brand == 'Weintek') && ($r->diagonal == 7) && ($r->model !== 'MT8071iER')) {
    $mqtt = '<tr><td>' . $im . '</td><td ' . $he . '><a class="ramkaoncart" href="#ui-id-5">Рамка для панели</a></td></tr>';
}
...
if (in_array($r->series, array("cMT-X"))) {
    $webview = '
<tr>
<td width="30" style="vertical-align: top;"><img src="/images/tick.png" width="15"></td>
<td class="hl_text" height="60">'
        . 'Управление панелью оператора через веб-браузер, без установки специального ПО (технология WebView).'
        . '</td></tr>';
}

if (in_array($r->model, array("cMT3072X","cMT3072XH","cMT3092X","cMT3152X","cMT3162X"))) {
    $plc_web_browser = '
<tr>
<td width="30" style="vertical-align: top;"><img src="/images/tick.png" width="15"></td>
<td class="hl_text" height="40">'
        . 'Веб браузер ПЛК (PLC Web browser) для подключения к встроенному веб-серверу ПЛК'
        . '</td></tr>';
}else $plc_web_browser = "";
...
if (!empty($r->ram)) {
  if($r->ram > 999){
    ...
    $ram = "<tr><td class=par_name1 >ОЗУ</td><td class=par_val1 >$r->ram</td></tr>";
}

if (!empty($r->flash)) {

/*  if($r->flash > 999){
    $r->flash = $r->flash / 1000 . " Гб";
  }else{
      $r->flash = $r->flash . " Мб";
  }*/
    $flash = "<tr><td class=par_name1 >Flash ( встроенный )</td><td class=par_val1 >$r->flash Гб</td></tr>";
}
;
if (!empty($r->rtc)) {
...
if ($r->index < 552){
    ...
    if (!empty($r->mpi)){
        $mpi = $us_tr_l . 'Поддержка MPI' . $us_tr_l1 . $r->mpi . '</td></tr>';
    }
}
    ...
    if (!empty($r->mysql_support) and $r->mysql_support != '') {
        $mysql_support = $us_tr_l . 'Подключение к серверу баз данных</td>' . $us_tr_l1 . $r->mysql_support . '</td></tr>';
    } else {
        $mysql_support = '';
    };
...
if ((($r->os != "") or ($r->type != 'monitor')) and $r->type != 'monitor_cloud_hmi') {
 ...
 <tr><td class=par_name1>ПО для разработки проектов</td><td class=par_val1 >$r->software</td></tr>";
        ...
        <?
        if ($r->codesys == "build_in"): ?>
            <tr>
                <td class=par_name1>Поддержка CODESYS</td>
                <td class=par_val1>Есть</td>
            </tr>
        <? elseif ($r->codesys == "optional"):?>
            <tr>
                <td class=par_name1>Поддержка CODESYS</td>
                <td class=par_val1>Опционально</td>
            </tr>
        <?endif;?>
        <tr>
...
$komplect = '';
if (($r->diagonal == '7' and $r->model !== 'MT8071iER') or ($r->diagonal >= 9 and $r->diagonal < 10) or ($r->diagonal >= 12 and $r->diagonal < 13)) {
...
if ($GLOBALS["net"] == 0) {
    ...
    if (isset($re[0]) and $re[0] > 0) {
        ...
        $t = date("d-m-Y", $re[1]);
        $fs = (sprintf("%u", $re[0]) + 0) / 1000;
    } elseif (($r->install_doc != '') AND isset($re1[0]) AND ($re1[0] > 0)) {
        $ne = 2;
        $t = date("d-m-Y", $re1[1]);
        $fs = (sprintf("%u", $re1[0]) + 0) / 1000;
    } elseif (($r->install_doc != '') AND isset($re1[0]) AND ($re2[0] > 0)) {
        $ne = 3;
        $t = date("d-m-Y", $re2[1]);
...
if ($r->diagonal == '7' && $r->model !== 'MT8071iER') {
