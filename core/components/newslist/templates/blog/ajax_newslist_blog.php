...
if (!$do_not_show_pager) {
    $rows = get_rows_from_table("`blog`", "", $filter_str, "id DESC", $offset . "," . $number_elements_per_page);
}else{
    $rows = get_rows_from_table("`blog`", "", $filter_str, "id DESC");
}
if (empty($rows)){
    $rows = get_rows_from_table("`blog`", "", $filter_str, "id DESC", "0," . $number_elements_per_page);
    $current_page = 1;
}
...
if (!$do_not_show_pager) {
    $arr_pager_items = get_pager("blog", $filter_str, $number_elements_per_page, $current_page);
...
$out["buffer"] = $buffer;
$out["products"] = $rows;
$out["all_brands"] = get_all_brands("blog");
$out["brands"] = $brands_out;
$out["pager_items"] = $arr_pager_items;
$out["current_page"] = $current_page;
