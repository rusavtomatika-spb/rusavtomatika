...
if (!$do_not_show_pager) {
    $rows = get_rows_from_table("`news`", "", $filter_str, "date DESC", $offset . "," . $number_elements_per_page);
} else {
    $rows = get_rows_from_table("`news`", "", $filter_str, "date DESC");
}
foreach ($rows as $key => $row) {
    $rows[$key]['date'] = date('d.m.Y', strtotime($row['date']));
    $rows[$key]['link'] = "/news/".$row['sys_name'] . "/";
...
function get_list_of_images($id)
{
    ...
    //
    if (file_exists($ROOT_FOLDER_FOR_IMAGES . $id)) {
