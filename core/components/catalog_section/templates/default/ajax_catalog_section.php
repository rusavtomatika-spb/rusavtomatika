...
} elseif (isset($_COOKIE['section_catalog_view_sort']) and $_COOKIE['section_catalog_view_sort'] != '') {
    $sort_mode = $_COOKIE['section_catalog_view_sort'];
} else {
...
switch ($sort_mode) {
    case "products_selected_by_query":
        ...
        foreach ($arr_products as $product) {
            if ($product["currency"] == "RUR" and $usd_currency > 0) {
            ...
            foreach ($arrSeries as $key => $brand) {
                if (in_array($key, $arBrands) or count($arBrands) == 0) { ?>
                    <div class='brand_logo_wrapper'>
                        ...
                        <div class="brand_name font_md"><?= $brand[0]['brand']["name"] ?></div>
                    </div>
                    <?php
                    foreach ($brand as $series) {

                        if (isset($only_series) and $only_series != "" and $only_series != $series["name"]) continue;
...
?>
    <div id="ajax_message" style="display: none"><?= $ajax_message ?></div>
<?php
    ...
    if (isset($row[0])) {
        ...
        if ($seo_data["active"] == "1") {
            ...
            if (isset($seo_data["UPPER_SEO_TEXT"])) {
                ...
                ?>
                <div id="component_seo_UPPER_SEO_TEXT"><?=$UPPER_SEO_TEXT?></div>
                <?php
            }
            if (isset($seo_data["LOWER_SEO_TEXT"])) {
                $LOWER_SEO_TEXT = $seo_data["LOWER_SEO_TEXT"];
                ?>
                <div id="component_seo_LOWER_SEO_TEXT"><?=$LOWER_SEO_TEXT?></div>
                <?php
            }
