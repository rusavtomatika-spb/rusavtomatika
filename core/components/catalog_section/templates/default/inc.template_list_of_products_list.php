    ...
    foreach ($series["products"] as $product) {
        ...
        <tr>
            <td class="td_preview_image">
                <a href="<?= $product["link_detail_page"]; ?>">
                    <div class="preview_image"
                         style="background-image:url('<?= $arSettings['path_to_product_images'] . mb_strtolower($product["brand"]) . "/" . mb_strtolower($product["type"]) . "/" . $product["model"] ?>/md/<?= $product["model"] ?>_1.png');">
                    </div>
                </a>
            </td>
            ...
            <td class="td_short_description">
                <a href="<?= $product["link_detail_page"]; ?>">
                    <?= $product["preview_text"];  ?> <div><?= $product["preview_text_extra"]; ?></div>
                </a>
            </td>
            <td class="td_onstock">
                <a href="<?= $product["link_detail_page"]; ?>">
                    ...
                    if (isset($product['retail_price']) and intval($product['retail_price']) > 0) {
                        ...
                        switch ($product['currency']) {
                            case 'USD':
                                ...
                                if ($usd_currency) {
                                    ?>
                                    <div class="rub_price"><? echo intval($product['retail_price'] * $usd_currency); ?> р.
                                    </div>
            ...
            <td class="td_buttons">
                <div class="buttons_panel">
                    <button title="Добавить в закладки" class="favorite">В закладки</button>
                    <button title="Добавить в сравнение" class="compare">В сравнение</button>
                    <button title="Заказать" class="cart">Заказать</button>
                </div>
            </td>
        </tr>
    <? }
