var app = new Vue({
    ...
    methods: {
        ...
        add_too_box(e) {
            ...
            if (button.attributes['data-model'].value != '') {
                ...
                switch (box) {
                    ...
                    case 'cart':
                        if (this.cart.indexOf(model) == -1) {
                            $(button).addClass('active').html("¬ заказе");
                            ...
                            $(button).removeClass('active').html("¬ заказ");
        ...
        getUrlVar() {
            ...
            var arrayVar = []; // массив дл€ хранени€ переменных
            var valueAndKey = []; // массив дл€ временного хранени€ значени€ и имени переменной
            var resultArray = []; // массив дл€ хранени€ переменных
            arrayVar = (urlVar.substr(1)).split('&'); // разбираем урл на параметры

            //if (arrayVar[0] == "") return false; // если нет переменных в урле
            for (let i = 0; i < arrayVar.length; i++) { // перебираем все переменные из урла
                if (arrayVar[i] == "") continue;
                valueAndKey = arrayVar[i].split('='); // пишем в массив им€ переменной и ее значение
                resultArray[valueAndKey[0]] = valueAndKey[1]; // пишем в итоговый массив им€ переменной и ее значение
            }
