...
<body>
<h1>Обновление раздела ДОКУМЕНТЫ</h1>
<p><? echo (date("H:i:s")); ?></p>
<br>
<br>
        ...
        if ( UR_exists( $src_url ) !== false ) {
          ...
          if ( $section == "chap" ) {
            if ( UR_exists( $src_url ) !== false ) {
              ...
              //                            echo "dest_path2chap: " . $dest_path2chap . " Section: " . $section . "<br>";
            }
          }
        }
      } else {
...
unset($tags,$src);
  ...
  $bad_words = "/(Chinese|japanese|HMI\sPin\sAssignment|RoHS|Connection\sGuide)/i"; //стоп-слова в названии
  $bad_urls = "/(cht|jap|jpn|_jpn|_cht|_tw|_jp|MT500|.htm)/i"; //стоп-слова в URL
...
$bad_cat = "/(Certificate)/i"; //гуд-слова в URL
    ...
    if ( ( !preg_match( $bad_words, $catalog[ 'title' ] ) ) && ( !preg_match( $bad_cat, $catalog[ 'url' ] ) ) && ( preg_match( $good_cat, $catalog[ 'category' ] ) ) ) {
//    if ( ( !preg_match( $bad_words, $catalog[ 'title' ] ) )) {
	...
	$descr='';
	$sort='2000';
      $put = $param = preg_replace( '/^(https?:\/\/dl.weintek.com\/public\/|http?:\/\/dl.weintek.com\/public\/|ftps?:\/\/dl.weintek.com\/public\/)/', '', $put );
	if (preg_match('/FCK_Upload/',$url)) $put = $param='Drawing';
	  if (preg_match('/Datasheet/i',$param)) {$dtype='Datasheet';$sort='100';}
	  if (preg_match('/Installation/i',$param)) {$dtype='Installation';$sort='200';}
	  if (preg_match('/Brochure/i',$param)) {$dtype='Brochure';$sort='600';}
	  if (preg_match('/UserManual/i',$param)) {$dtype='UserManual';$sort='300';}
	  if (preg_match('/Utility/i',$param)) {$dtype='Utility';$sort='700';}
	  if (preg_match('/Drawing/i',$param)) {$dtype='Drawing';$sort='500';}
	  if (preg_match('/Part_Sample/i',$param)) {$dtype='Part_Sample';$sort='1000';}
	  if (preg_match('/PLC_Sample/i',$param)) {$dtype='PLC_Sample';$sort='1100';}
	  if (preg_match('/Macro_Sample/i',$param)) {$dtype='Macro_Sample';$sort='1200';}
	  if (preg_match('/System_Sample/i',$param)) {$dtype='System_Sample';$sort='1300';}
	  if (preg_match('/Demo/i',$param)) {$dtype='Demo';$sort='900';}
	  if (preg_match('/Library/i',$param)) {$dtype='Library';$sort='1400';}
	  if (preg_match('/SDK/i',$param)) {$dtype='SDK';$sort='710';}
	  if (preg_match('/API/i',$param)) {$dtype='API';$sort='720';}
	  if (preg_match('/3D Model/i',$cate)) {$dtype='3D_model';$sort='400';}
	  if (preg_match('/2D Cad/i',$cate)) {$dtype='2D_CAD';$sort='500';}
	  if (preg_match('/EasyBuilder Pro User Manual/i',$title)) {$dtype='ebpro';$sort='1500';}
...
echo "<br>1) В массиве files ".count($files);
/////////////////////////
$i = 0;
foreach ( $files as $item ) {
  ...
  $title = preg_replace( '/(.+)(Installation)(.*)/i', 'Инструкция по установке $1$2$3', $title );
  $title = preg_replace( '/(.+)(Brochure)(.*)/i', 'Брошюра $1$2$3', $title );
  $title = preg_replace( '/(.+)(User[\s]*Manual)(.*)/i', 'Руководство пользователя $1$2$3', $title );
  //echo $label . "<hr>";
	if (strpos( $title, "cMT3108XP") !== false) {$label = 'cMT3108XP,'.$label;}
	if (strpos( $title, "cMT3108XH" ) !== false) {$label = 'cMT3108XH,'.$label;}
	if (strpos( $fname, "cMT-G01X_" ) !== false) {$title = preg_replace( '/cMT-G01\s/i', 'cMT-G01X ', $title );}
  $files[ $i ][ 'label' ] = $label;
  $files[ $i ][ 'title' ] = $title;
  $i++;
}
  ...
  foreach ( $alltags as $catalog ) {
	...
	if (preg_match('/FCK_Upload/',$url)) $put = $param='Drawing';
      ...
      if ( preg_match( "/DocumentEB/", $label ) ) {
        ...
        $section = "cmt";
      } else if ( preg_match( "/MT-*8[0-9]{3}iP/", $label ) ) {
        $section = "mt8000ip";
...
unset( $alltags );
  ...
  file_put_contents( $docs_result, json_encode( $items, JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE ) );
